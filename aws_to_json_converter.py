...
def aws_to_json(file_path, header=True, validation=True, layers=False, rules=False, entities=False):
    ...
    # Create a disclamer
    disclamer = {
        'disclamer': 'This file was automatically generated by the Ancient Warfare Structure Pack AWS to JSON converter.',
        'date': date,
    ...
    json_file_path = file_path.replace('.aws', '.json')
    json_file_path = json_file_path.replace("structures", "structures_jsons")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Convert AWS files to JSON files.')

    # Add arguments
    parser.add_argument('file_path', type=str, help='The path to the AWS file to convert.')
    parser.add_argument('-hdr', '--header', action='store_true', help='Parse the header of the AWS file.', default=True)
    parser.add_argument('-vld', '--validation', action='store_true', help='Parse the validation of the AWS file.', default=True)
    parser.add_argument('-lyr', '--layers', action='store_true', help='Parse the layers of the AWS file.', default=False)
    parser.add_argument('-rls', '--rules', action='store_true', help='Parse the rules of the AWS file.', default=False)
    parser.add_argument('-ent', '--entities', action='store_true', help='Parse the entities of the AWS file.', default=False)

    # Parse arguments
    args = parser.parse_args()

    aws_to_json(args.file_path, args.header, args.validation, args.layers, args.rules, args.entities)
